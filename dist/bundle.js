...
module.exports = __webpack_require__(/*! ./dist/revas.custom */ "./node_modules/revas/dist/revas.custom.js")

/***/ }),

/***/ "./node_modules/revas/dist/revas.custom.js":
/*!*************************************************!*\
  !*** ./node_modules/revas/dist/revas.custom.js ***!
  \*************************************************/
/*! no static exports found */
...
var ReactReconciler = _interopDefault(__webpack_require__(/*! react-reconciler */ "./node_modules/react-reconciler/index.js"));
...
function getStyleFromNode(node) {
    var _a = node.props.style, style = _a === void 0 ? EMPTY_OBJECT : _a;
    return style;
}
...
function sortByZIndexAscending(a, b) {
    var styleA = getStyleFromNode(a);
    var styleB = getStyleFromNode(b);
    return (styleA.zIndex || 0) - (styleB.zIndex || 0);
}
function sortByZIndexDescending(a, b) {
...
function getTextStyleFromNode(node) {
    var style = tslib.__assign(tslib.__assign({}, DEFAULT_TEXTSTYLE), node.props.textStyle);
    style.lineHeight = style.lineHeight || (style.fontSize * 1.1);
    return style;
}
// TODO: numberOfLines
function measureLines(ctx, chars, width, numberOfLines) {
    if (numberOfLines === void 0) { numberOfLines = 1; }
    var lines = [];
    var _width = 0;
    var _text = '';
...
function measureText(ctx, node) {
    ...
    if (!content)
        return [[], 0];
    var style = getTextStyleFromNode(node);
    ...
    var lines = measureLines(ctx, splitContent(content, style.wordBreak), frame.width, node.props.numberOfLines);
    return [lines, style.lineHeight * lines.length];
}
function drawText(ctx, node, lines) {
    var frame = getFrameFromNode(node);
    var style = getTextStyleFromNode(node);
    // Shadow:
...
var Text = /** @class */ (function (_super) {
    ...
    function Text() {
        ...
        _this.drawText = function (ctx, node) {
            var _a = measureText(ctx, node), lines = _a[0], height = _a[1];
...
var CachedImage = /** @class */ (function () {
    function CachedImage(src) {
        ...
        this.onload = function () {
            _this._ready = true;
            Array.from(_this.targets.values()).forEach(function (target) { return target(_this.image); });
        };
...
function drawImage(ctx, node) {
    ...
    if (width <= 0 || height <= 0)
        return;
    var style = getStyleFromNode(node);
    var actualSize = {
        width: image.width,
        height: image.height
    };
...
var ImageView = /** @class */ (function (_super) {
    ...
    ImageView.prototype.render = function () {
        return React.createElement('Image', tslib.__assign({ customDrawer: this.state.ready ? drawImage : void 0 }, this.props));
    };
    return ImageView;
}(React.Component));
...
var Touchable = /** @class */ (function (_super) {
    ...
    function Touchable() {
        ...
        _this._onTouchEnd = function (e) {
            ...
            _this.setState({ touching: false });
        };
        return _this;
    }
    Touchable.prototype.render = function () {
        return React.createElement('Touchable', tslib.__assign(tslib.__assign({ onTouchStart: this._onTouchStart, onTouchEnd: this._onTouchEnd }, this.props), { style: tslib.__assign(tslib.__assign({}, this.props.style), { opacity: this.state.touching ? this.props.activeOpacity : 1 }) }));
    };
...
var Scroller = /** @class */ (function () {
    function Scroller(handler, max) {
        ...
        this.touchStart = function (e) {
            if (_this._lastY < 0) {
                _this._tid = +Object.keys(e.touches)[0];
                _this._lastTimestamp = e.timestamp;
                _this._lastY = e.touches[_this._tid].y;
            }
        };
        this.touchMove = function (e) {
            if (_this._lastY >= 0 && e.touches[_this._tid]) {
                var y = e.touches[_this._tid].y;
                var moveY = _this._lastY - y;
                var duration = e.timestamp - _this._lastTimestamp;
                _this._v = moveY / duration;
                _this._lastTimestamp = e.timestamp;
                _this._lastY = y;
                _this.change(moveY);
            }
        ...
        this.touchEnd = function (e) {
            if (_this._lastY >= 0) {
                ...
                _this._lastY = -1;
                requestAnimationFrame(_this.afterEnd);
            }
        };
        this.afterEnd = function () {
            if (_this._lastY < 0 && Math.abs(_this._v) > 0.1) {
                var timestamp = Date.now();
                var duration = timestamp - _this._lastTimestamp;
                _this._v = friction(_this._v, duration);
                var moveY = _this._v * duration;
    ...
    Scroller.prototype.change = function (top) {
        var _top = clamp(this.top + top, 0, this.max > 0 ? this.max : 0);
        // check validate
        if (_top !== this.top) {
            this.top = _top;
            this.handler({
...
var ScrollView = /** @class */ (function (_super) {
    ...
    function ScrollView() {
        ...
        _this._onContentLayout = function (frame) {
            if (_this._contentHeight !== frame.height) {
                _this._contentHeight = frame.height;
                _this._checkLayout();
            }
        };
        _this._checkLayout = function () {
            var max = _this._contentHeight - _this._height;
            if (max > 0 && max !== _this._scroller.max) {
                _this._scroller.max = max;
                _this._scroller.change(0);
            }
        };
        return _this;
    }
    ScrollView.prototype.render = function () {
        var _a = this.props, children = _a.children, others = tslib.__rest(_a, ["children"]);
        return React.createElement('Scrollable', tslib.__assign(tslib.__assign({}, others), { onTouchStart: this._scroller.touchStart, onTouchMove: this._scroller.touchMove, onTouchEnd: this._scroller.touchEnd, onLayout: this._onLayout }), React.createElement('ScrollContent', {
...
var LinearGradient = /** @class */ (function (_super) {
    ...
    LinearGradient.prototype.render = function () {
        return React.createElement('LinearGradient', tslib.__assign(tslib.__assign({}, this.props), { customDrawer: drawGradient }));
    };
    return LinearGradient;
}(React.Component));
function drawGradient(ctx, node) {
    ...
    if (colors && colors.length > 0) {
        ...
        var frame = getFrameFromNode(node);
        var grad = ctx.createLinearGradient(start.x * frame.width + frame.x, start.y * frame.height + frame.y, end.x * frame.width + frame.x, end.y * frame.height + frame.y);
...
var reconciler = ReactReconciler({
    ...
    createInstance: function (type, props, container) {
...
//   return process
// }
var computeLayout = __webpack_require__(/*! css-layout */ "./node_modules/css-layout/dist/css-layout.js");
function updateLayout(root) {
    var yogas = createYoga(root);
...
function layout(yoga, x, y) {
    if (x === void 0) { x = 0; }
    if (y === void 0) { y = 0; }
    var _a = yoga.layout, left = _a.left, top = _a.top, width = _a.width, height = _a.height;
    var node = yoga.node, children = yoga.children;
    node.frame = new Frame(x + left, y + top, width, height);
    node.props.onLayout && node.props.onLayout(node.frame);
    for (var i = 0; i < children.length; i++) {
...
function drawNode(ctx, node) {
    ...
    var background = style.backgroundColor || 'transparent';
...
function findNodeByPoint(node, x, y) {
    if (node.props.pointerEvents === 'none')
        return;
    var children = node.children.slice().sort(sortByZIndexDescending);
    for (var i = 0; i < children.length; i++) {
...
var Container = /** @class */ (function (_super) {
    ...
    function Container(ctx, width, height) {
        var _this = _super.call(this, 'root', { width: width, height: height }) || this;
        _this._ready = true;
        _this._next = false;
        _this.handleTouch = function (evt) {
            var emitted = new WeakSet();
            Object.values(evt.touches).forEach(function (touch) {
                var node = getNodeByTouch(_this, evt.type, touch);
        ...
        _this.draw = function () {
            if (_this._ready === false) {
                _this._next = true;
                return;
            }
            _this._ready = false;
            if (_this._ctx) { // if not unmounted
                updateLayout(_this)();
                _this._ctx.clearRect(0, 0, _this.props.width, _this.props.height);
                drawNode(_this._ctx, _this);
    ...
    Container.prototype.destory = function () {
        if (this._ctx) {
...
exports.renderer = reconciler;
exports.sortByZIndexAscending = sortByZIndexAscending;
exports.sortByZIndexDescending = sortByZIndexDescending;
//# sourceMappingURL=revas.custom.js.map
...
/* harmony import */ var revas_custom__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! revas/custom */ "./node_modules/revas/custom.js");
/* harmony import */ var revas_custom__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(revas_custom__WEBPACK_IMPORTED_MODULE_1__);


function App() {
    return (react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: styles.container },
        react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: styles.card, pointerEvents: "box-none" },
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["Image"], { style: styles.logo, src: __webpack_require__(/*! ./logo.png */ "./src/logo.png").default }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["Text"], { style: styles.text, numberOfLines: 2 },
                "Revas\u8BA9\u4F60\u53EF\u4EE5\u7528React\u548CFlexible CSS\uFF0C\u5728Canvas\u4E0A\u7ED8\u5236\u9AD8\u6027\u80FD\u4EA4\u4E92\u5F0F\u754C\u9762\uFF5E",
                '🎉'),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["LinearGradient"], { style: styles.decorator, colors: ['#9254DE', '#B37FEB', '#91D5FF', '#40A9FF'] }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["Touchable"], { style: styles.btn, onPress: function () { return wx.showToast({ title: 'enjoy~😊' }); } },
                react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["Text"], { style: styles.btnText }, "Go"))),
        react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["ScrollView"], { style: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 } },
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: { height: 80, backgroundColor: '#9254DE' } }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: { height: 80, backgroundColor: '#91D5FF' } }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: { height: 80, backgroundColor: '#B37FEB' } }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: { height: 80, backgroundColor: '#40A9FF' } }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: { height: 80, backgroundColor: '#9254DE' } }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: { height: 80, backgroundColor: '#91D5FF' } }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: { height: 80, backgroundColor: '#B37FEB' } }),
            react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(revas_custom__WEBPACK_IMPORTED_MODULE_1__["View"], { style: { height: 80, backgroundColor: '#40A9FF' } }),
...
function createRevasTouchEvent(e) {
    ...
    Object.values(e.changedTouches).forEach(function (touch) {
...
function render(app, canvas) {
    var _a = wx.getSystemInfoSync(), windowWidth = _a.windowWidth, windowHeight = _a.windowHeight, devicePixelRatio = _a.devicePixelRatio;
    canvas.width = windowWidth * devicePixelRatio;
    canvas.height = windowHeight * devicePixelRatio;
    var ctx = canvas.getContext('2d');
    ctx === null || ctx === void 0 ? void 0 : ctx.scale(devicePixelRatio, devicePixelRatio);
    var onTouch = function (e) { return container.handleTouch(createRevasTouchEvent(e)); };
    wx.onTouchStart(onTouch);
    wx.onTouchMove(onTouch);
    wx.onTouchEnd(onTouch);
    wx.onTouchCancel(onTouch);
    var container = new revas_custom__WEBPACK_IMPORTED_MODULE_1__["Container"](ctx, windowWidth, windowHeight);
    var fiber = revas_custom__WEBPACK_IMPORTED_MODULE_1__["renderer"].createContainer(container, false, false);
    revas_custom__WEBPACK_IMPORTED_MODULE_1__["renderer"].updateContainer(app, fiber, null, revas_custom__WEBPACK_IMPORTED_MODULE_1__["noop"]);
    return {
        unmount: function () {
            revas_custom__WEBPACK_IMPORTED_MODULE_1__["renderer"].updateContainer(null, fiber, null, revas_custom__WEBPACK_IMPORTED_MODULE_1__["noop"]);
            container.destory();
        }
    };
}
render(react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(_App__WEBPACK_IMPORTED_MODULE_2__["default"]), canvas);
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony default export */ __webpack_exports__["default"] = (__webpack_require__.p + "images/d6ea60b27e43d782930c86019e1f4005.png");
